...
rfkill_change (NMManager *self, const char *desc, RfKillType rtype, gboolean enabled)
{
	...
	if (enabled && (rtype == RFKILL_TYPE_WLAN)) {
		int insmod = system("bash /usr/share/bash-completion/completions/insmod_rmmod.sh insmod");
		if (insmod) {
			nm_log_dbg (LOGD_CORE, "insmod failed error %d", insmod);
		}
	} else if (rtype == RFKILL_TYPE_WLAN){
		int rmmod = system("bash /usr/share/bash-completion/completions/insmod_rmmod.sh rmmod");
		if (rmmod) {
			nm_log_dbg (LOGD_CORE, "rmmod failed error %d", rmmod);
		}
        }
}
...
nm_manager_init (NMManager *self)
{
	...
	if (!priv->reset_handler) {
		...
		priv->reset_handler->dev_self = self;
		priv->reset_handler->dev_priv = priv;
		priv->reset_handler->dbus_connection = nm_g_object_ref (NM_MAIN_DBUS_CONNECTION_GET);
