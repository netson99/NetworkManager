...
constructed (GObject *object)
{
	...
	/*if (!priv->listen_handler) {
		...
		priv->listen_handler->dev_wifi_priv = priv;
		priv->listen_handler->dbus_connection = nm_g_object_ref (NM_MAIN_DBUS_CONNECTION_GET);
		priv->listen_handler->periodic_source_id = &priv->periodic_source_id;
		priv->listen_handler->periodic_update_cb = periodic_update_cb;
		priv->listen_handler->set_powersave = js_set_powersave;
